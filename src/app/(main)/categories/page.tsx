...
function Categories() {
  ...
  // if(delSuccess) toastStore.addToast(true, "Berhasil menghapus category");
  // if(isError) toastStore.addToast(false, delError.message);
  ...
  return (
    <>
      ...
      <div className="flex m-6 mx-10 p-3 px-6 items-center rounded-2xl justify-end shadow-sm border-black/10 border [&_p]:lg:text-lg [&_p]:font-bold sticky top-0 bg-white z-10">
        <button className="bg-black text-white p-2 px-4 rounded-full cursor-pointer flex gap-2" onClick={() => modalStore.openCreate()}>
          <span className="text- lg:text-base">tambah kategori</span>
              ...
              {data?.length === 0 || !data ?
                ...
                <>
                  ...
                  ">
                        ...
                        <button onClick={() => deleteCat(item.id)} className="bg-red-700 p-1.5">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
